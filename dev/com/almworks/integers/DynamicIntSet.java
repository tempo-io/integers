...
public class DynamicIntSet {
  ...
  private static final int[] EMPTY_KEYS = new int[] { Integer.MIN_VALUE };
  ...
  public DynamicIntSet(int initialCapacity) {
    initialCapacity += 1;
  ...
  private void initNode(int x, int key) {
    myKeys[x] = key;
    myLeft[x] = 0;
    myRight[x] = 0;
  }
  ...
  /** @return {@link Integer#MIN_VALUE} in case the set is empty */
  public int getMax() {
  ...
  private boolean add0(int key, int[] ps) {
    int x = myRoot;
    // Parents stack top + 1
    int psi = 0;
  ...
  final StringBuilder dumpArrays(int problematicNode) {
    ...
    int longestKey = max(abs(getMax()), abs(getMin()));
