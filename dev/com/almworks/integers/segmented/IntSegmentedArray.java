...
public class IntSegmentedArray extends AbstractWritableIntList implements Cloneable {
  ...
  private final IntSegmentedArrayEnvironment myEnv;
   ...
   * <p/>
   * for usage, see {@link #writeValue(int, int)}
   */
  ...
  public IntSegmentedArray() {
    this(IntSegmentedArrayHeapEnvironment.INSTANCE);
  }

  public IntSegmentedArray(IntSegmentedArrayEnvironment env) {
    myEnv = env;
    assert !IntegersDebug.CHECK || checkInvariants();
  }

  public static IntSegmentedArray create(IntList values) {
    IntSegmentedArray array = new IntSegmentedArray();
    array.addAll(values);
    return array;
  }
  ...
  public void insertAll(int index, IntList values, int sourceIndex, int count) {
    if (values == null || count <= 0)
      return;
    if (values instanceof IntSegmentedArray) {
      insertSegmented(index, (IntSegmentedArray) values, sourceIndex, count);
  ...
  public void setAll(int index, IntList values, int sourceIndex, int count) {
    ...
    if (values instanceof IntSegmentedArray) {
      copySegmented(index, (IntSegmentedArray) values, sourceIndex, count);
    } else if (values instanceof SubList) {
      SubList sublist = (SubList) values;
      if (sourceIndex + count > sublist.size())
        throw new IndexOutOfBoundsException(sourceIndex + " " + count + " " + values);
  ...
  public IntSegmentedArray clone() {
    return clone(0, size());
  }

  public IntSegmentedArray clone(int from, int to) {
    assert !IntegersDebug.CHECK || checkInvariants();
    checkRange(from, to);
    if (from >= to)
      return new IntSegmentedArray(myEnv);
    try {
      assert mySegmentCount > 0 : this;
      IntSegmentedArray r = (IntSegmentedArray) super.clone();
  ...
  private void insertSegmented(int index, IntSegmentedArray array, int sourceIndex, int count) {
    assert !IntegersDebug.CHECK || checkInvariants();
  ...
  private void copySegmented(int targetIndex, IntSegmentedArray array, int sourceIndex, int length) {
    int dp = myLeftOffset + targetIndex;
    int di = dp >> mySegmentBits;
    dp &= mySegmentMask;
  ...
  public void setRange(int from, int to, int value) {
    ...
    int len = to - from;
    setAll(from, IntCollections.repeat(value, len), 0, len);

    assert !IntegersDebug.CHECK || checkInvariants();
  }

  private void writeValue(int absIndex, int value) {
  ...
  public void apply(int from, int to, IntToInt function) {
    ...
    checkRange(from, to);
    setAll(from, IntCollections.map(function, this), from, len);
    assert !IntegersDebug.CHECK || checkInvariants();
  }
  ...
  private class SegmentedIterator extends AbstractIntIterator implements WritableIntListIterator {
    ...
    public void removeRange(int offsetFrom, int offsetTo) throws NoSuchElementException {
      ...
      IntSegmentedArray.this.removeRange(f, t);
      myNext = f;
      myTo -= (t - f);
    ...
    public String toString() {
      return myNext + "[" + myFrom + ";" + myTo + ") " + IntSegmentedArray.this;
    }
