 ...
 */
public class IntSameValuesList extends AbstractWritableIntList {
  /**
   * Maps index to the value that starts at that index
   */
  private IntIntListMap myMap;

  public IntSameValuesList() {
    this(new IntIntListMap());
  }

  public IntSameValuesList(IntIntListMap hostMap) {
    if (!hostMap.isEmpty()) {
  ...
  /**
   * Creates IntSameValuesList containing the specified values which are repeated the specified number of times.
   ...
   * @return IntSameValuesList containing the specified values which are repeated the specified number of times.
   * @throws IllegalArgumentException if {@code counts.size < values.size}
   */
  public static IntSameValuesList create(IntSizedIterable values, IntIterable counts) throws IllegalArgumentException {
    IntSameValuesList list = new IntSameValuesList();
    if (values.size() != 0) {
  ...
  /**
   * @return IntSameValuesList containing the specified values
   * @see #create(com.almworks.integers.IntSizedIterable, com.almworks.integers.IntIterable)
   */
  public static IntSameValuesList create(IntSizedIterable values) throws IllegalArgumentException {
    return create(values, repeat(1));
  }

  public void init(IntSizedIterable values, IntIterable counts) throws IllegalArgumentException {
    assert size() == 0;

    IntIntListMap.ConsistencyViolatingMutator m = myMap.startMutation();
    int pos = 0, i = 0;
    int prevValue = 0;
    for (IntIntIterator it : IntIntIterators.pair(counts, values)) {
      int curCount = it.left();
      int curValue = it.right();
      if (curCount < 0) {
  ...
  public void add(int value) {
    ...
    int sz = size();
    int msz = myMap.size();
    int lastValue = msz == 0 ? value + 1 : myMap.getValueAt(msz - 1);
    if (value != lastValue) {
  ...
  public void insertMultiple(int index, int value, int count) {
    ...
    int prevValue = prevValueForFindIndex(ki, value);
    int sz = myMap.size();
  ...
  public void setRange(int from, int to, int value) {
    ...
    int prevValue = prevValueForFindIndex(fi, value);
  ...
  private int prevValueForFindIndex(int findIndex, int value) {
  ...
  private boolean checkInvariants() {
    ...
    if (myMap.size() == 0) {
      ...
      assert size > 0 : size;
      assert myMap.size() > 0;
      int lastValue = myMap.getValueAt(0) - 1;
      int lastKey = -1;
  ...
  public void sort(final WritableIntList... sortAlso) {
    final IntArray values = new IntArray(this.iterator());
  ...
  public void removeRange(int from, int to) {
    ...
    if (to == size) {
      ...
      int prevValue = prevValueForFindIndex(fi, followingValue);
  ...
  private final class SameValuesIterator extends WritableIndexIterator {
    private IntIntIterator myIterator;
    private int myValue;
    ...
    protected void sync() {
      ...
      if (p == -1) {
        ...
        myIterator = IntIntIterator.EMPTY;
      } else {
        if (p < 0) {
          p = -p - 2;
        }
