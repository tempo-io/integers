...
public class LongLongListMap extends AbstractWritableLongLongMap {
  ...
  public LongLongListMap(WritableLongList keys, WritableLongList values) {
    myKeys = keys;
    myValues = values;
  ...
  public LongLongListMap() {
    this(new LongArray(), new LongArray());
  }
  ...
  public LongLongIterator iterator(int from) {
    return iterator(from, size());
  }

  public LongLongIterator iterator(int from, int to) {
    checkMutatorPresence();
    return LongLongIterators.pair(myKeys.iterator(from, to), myValues.iterator(from, to));
  }

  public LongLongIterator iterator() {
    checkMutatorPresence();
    return new LongLongFailFastIterator(iterator(0)) {
      @Override
  ...
  /**
   * Enters this {@code LongLongListMap} into a mode in which consistency-breaking mutations are allowed.
   *
   * <p>While in this mode, usage of all of this {@code LongLongListMap}'s methods
   ...
   * {@code myMutator.commit()} brings this {@code LongLongListMap} back to its normal state.</p>
   * @throws IllegalStateException if this {@code LongLongListMap} is already in mutation state.
   */
  ...
  public class ConsistencyViolatingMutator {
    ...
    public void commit() {
      ...
      LongLongListMap.this.myMutator = null;
    }
  }
