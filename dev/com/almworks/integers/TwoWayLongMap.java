...
public class TwoWayLongMap {
  ...
  public void insertAllRo(LongList keys, LongFunction keyToVal) {
    insertAll(new LongArray(keys), keyToVal);
  }

  public void insertAll(WritableLongList keys, LongFunction keyToVal) {
    int m = keys.size();
  ...
  public void insertAll(WritableLongList keys, WritableLongList vals) throws IllegalArgumentException {
    ...
    for (int i = 0; i < m; ) {
      ...
      for (; i < m && insPoints.get(i) == ins; ++i) {
        ...
        if (ki >= 0) {
          assert false : key;
          throw new IllegalArgumentException("TwoWayLongMap is broken");
        }
        ki = -ki - 1;
        myKeys.insert(ki, key);
   ...
   * Memory: O(n). */
  public void transformVals(@NotNull LongFunction f) {
    transformVals(Long.MIN_VALUE, f);
  }

  /** Transforms each value using the specified function. Values are supplied in ascending order.<br/>
   * Memory: O(n). */
  public void transformVals(long valFrom, @NotNull LongFunction f) {
    int n = size();
  ...
  public void transformVals(@NotNull LongFunction2 f) {
    int n = size();
  ...
  public void transformKeys(LongFunction injection) throws NonInjectiveFunctionException {
    int n = size();
