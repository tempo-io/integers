...
public interface WritableIntObjMap<T> extends IntObjMap<T> {
   ...
   * @return this
   */
  WritableIntObjMap<T> add(int key, T value);
   ...
   */
  boolean putIfAbsent(int key, T value);
   ...
   */
  void putAll(IntObjIterable<T> entries);

  /**
   * @throws IllegalArgumentException if {@code keys.size() != values.size()}
   * @see #putAll(int[], Object[])
   */
  void putAll(IntSizedIterable keys, Collection<T> values);
   ...
   */
  void putAllKeys(IntIterable keys, Iterable<T> values);
