...
public final class IntArray extends AbstractWritableIntList {
  ...
  /**
   * Constructs a IntArray and adds all elements from {@code copyFrom}
   */
  ...
  /**
   * Constructs a IntArray and add all elements from {@code iterator}
   */
  ...
  /**
   * Constructs a IntArray that is backed by {@code hostArray}
   * See {@link IntArray#copy(int[])} }, {@link IntArray#create(int...)}.
   * */
  ...
  /**
   * Constructs a IntArray with the specified size {@code length} that is backed by {@code hostArray}
   * See {@link IntArray#copy(int[])}, {@link IntArray#create(int...)}.
  ...
  /**
   * @return IntArray containing elements from the specified {@code int[]} array
   */
  ...
  /**
   * @return IntArray with the specified size
   * containing elements from the specified {@code int[]} array
   */
  ...
  /**
   * @return IntArray containing elements from {@code iterable}
   */
  public static IntArray copy(@Nullable IntIterable iterable) {
    if (iterable == null) return new IntArray();
    return IntCollections.collectIterable(0, iterable);
  }

  /**
   * @return IntArray containing the specified elements.
   * See {@link IntArray#IntArray(int[])}, {@link IntArray#IntArray(int[], int)}.
   * */
  ...
  /**
   * May be used to convert the collection of {@code Int} to native int[] array:
   * {@code IntArray.create(values).extractHostArray()}.
   * @return IntArray containing elements from collection {@code values}
   */
  ...
  public void ensureCapacity(int expectedCapacity) {
    myArray = IntCollections.ensureCapacity(myArray, expectedCapacity);
  }
   ...
   * @param values sorted {@code IntList}
   * */
  public void retainSorted(IntList values) {
    assert values.isSorted();
    retain(values, true);
  }

  private void retain(IntList values, boolean valuesSortedStatus) {
    if (values.isEmpty()) {
      clear();
      return;
    }
    IntList sortedValues = valuesSortedStatus ? values : IntCollections.toSorted(false, values);
    int i = 0, i2;
   ...
   */
  public int addAllNotMore(IntList collection, int maxCount) {
   ...
   * @return number of added elements
   */
  public int addAllNotMore(IntIterable iterable, int maxCount) {
    int counter = 0;
   ...
   */
  public boolean removeSorted(int value) {
    assert isSorted();
   ...
   * <p>Unlike {@link IntCollections#removeAllAtSorted(WritableIntList, IntList)}, which in case of IntArray takes
   ...
   * @see com.almworks.integers.IntCollections#removeAllAtSorted(WritableIntList, IntList)
   */
  ...
  /**
   * Extracts from this array inner buffer with {@code int[]} values and clears this array.
   * @return int[] array with values from this array.
   */
   ...
   */
  public int getInsertionPoints(IntSizedIterable src, int[][] points) {
    ...
    for (int i = 0; i < srcSize; i++) {
      ...
      last = v;
      if (destIndex < size) {
        int k = IntCollections.binarySearch(v, myArray, destIndex, size);
        if (k >= 0) {
          // found
          destIndex = k;
          continue;
   ...
   * {@link IntArray#mergeWithSmall(IntList)} or {@link IntArray#mergeWithSameLength(IntList)}.
   * <p>If this array is smaller than the specified list,
   * it will copy the specified list and merge it with existing contents using {@link IntArray#mergeWithSmall(IntList)}.
   */
  public void merge(IntList src) {
    ...
    if (k < 10) {
      ...
      if (k < 10) {
        ...
        int[] small = myArray;
        myArray = IntCollections.ensureCapacity(EMPTY_INTS, size + srcSize);
        src.toNativeArray(0, myArray, 0, srcSize);
        updateSize(srcSize);
        mergeWithSmall(new IntArray(small, size));
      }
    } else {
      mergeWithSmall(src);
    }
   ...
   * <p>Prefer to use this method over {@link com.almworks.integers.IntArray#mergeWithSameLength(IntList)}
   * If {@code src.size()} is much smaller than {@code size()}. If they are close
   * use {@link com.almworks.integers.IntArray#mergeWithSameLength(IntList)}
   *
   * @param  src sorted list.
   * @see #merge(IntList)
   * */
  public void mergeWithSmall(IntList src) {
   ...
   * @see #mergeWithSmall(IntList)
   */
  public void mergeWithSmall(IntList src, int[][] insertionPoints) {
    if (src.isEmpty()) return;
    ...
    if (oldSize + srcSize > myArray.length) {
      ...
      int[] newDest = new int[Math.max(oldSize + srcSize, oldLength * 2)];
      int last = 0;
      int newDestSize = 0;
      int destIndex = 0;
      for (int i = 0; i < srcSize; i++) {
        ...
        if (destIndex < oldSize) {
          int k = IntCollections.binarySearch(v, myArray, destIndex, oldSize);
          if (k >= 0) {
            // found
            continue;
          }
   ...
   * it's better to use {@link com.almworks.integers.IntArray#mergeWithSmall(IntList)}
   *
   * @param  src sorted list of numbers (set)
   * */
  public void mergeWithSameLength(IntList src) {
  ...
  private void merge0inPlace(IntList src, int srcSize, int totalLength) {
    // in place
  ...
  private void merge0withReallocation(IntList src, int srcSize, int totalLength) {
