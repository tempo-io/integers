...
public class DynamicLongSetEU implements LongIterable {
  ...
  public void addAll(DynamicLongSetEU keys) {
    modified();
    Entry[] ps = prepareAdd(keys.size());
   ...
   * */
  private void rotateLeft(Entry x, Entry p) {
    Entry y = x.r;
    x.r = y.l;
    y.l = x;
    if (p == NIL) myRoot = y;
   ...
   * */
  private void rotateRight(Entry x, Entry p) {
    Entry y = x.l;
    x.l = y.r;
    y.r = x;
    if (p == NIL) myRoot = y;
    ...
    while (x != myRoot && x.color == BLACK) {
      ...
      if (parentOfX.l == x) {
        ...
        if (w.color == RED) {
          ...
          w.color = BLACK;
          parentOfX.color = RED;
          rotateLeft(parentOfX, getLastOrNil(parentsStack, xsi-1));
          parentsStack[xsi] = w;
        ...
        if (w.color == RED) {
          ...
          w.color = BLACK;
          parentOfX.color = RED;
          rotateRight(parentOfX, getLastOrNil(parentsStack, xsi-1));
          parentsStack[xsi] = w;
          parentsStack[++xsi] = parentOfX;
          w = parentOfX.l;
        }
