...
public interface WritableLongIntMap extends LongIntMap {
   ...
   * @return this
   */
  WritableLongIntMap add(long key, int value);
   ...
   * @see #putAll(LongSizedIterable, IntSizedIterable)
   */
  void putAll(LongIntIterable entries);

  /**
   * @throws IllegalArgumentException if {@code keys.size() != values.size()}
   * @see #putAll(long[], int[])
   */
  void putAll(LongSizedIterable keys, IntSizedIterable values);
   ...
   */
  void putAll(long[] keys, int[] values);
   ...
   */
  void putAllKeys(LongIterable keys, IntIterable values);
