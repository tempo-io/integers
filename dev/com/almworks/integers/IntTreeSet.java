...
// CODE GENERATED FROM com/almworks/integers/PTreeSet.tpl
...
public class IntTreeSet extends AbstractWritableIntSet implements WritableIntSortedSet {
  ...
  private static final int[] EMPTY_KEYS = new int[] { NIL_DUMMY_KEY };
  ...
  // used in fromSortedIntIterable(). A new myKeys array is created in this method, and its size is
  // the size of the given IntIterable multiplied by this constant (additional space for new elements to be added later).
  ...
  /**
   * This enum is used in {@link IntTreeSet#compactify(IntTreeSet.ColoringType)} and
   * {@link IntTreeSet#initFromSortedUnique(IntIterable, int, com.almworks.integers.IntTreeSet.ColoringType)}
  ...
  /**
   * Constructs an empty <tt>IntTreeSet</tt> with the specified initial capacity.
   * */
  public IntTreeSet(int initialCapacity) {
    if (initialCapacity < 0) throw new IllegalArgumentException();
    initialCapacity += 1;
    myBlack = new BitSet(initialCapacity);
    myRemoved = new BitSet(initialCapacity);
    init();
    myKeys = new int[initialCapacity];
   ...
   * @return new {@code IntTreeSet} with elements from {@code src} with the specified capacity and coloring type.
   */
  public static IntTreeSet createFromSortedUnique(IntIterable src, int capacity, ColoringType coloringType) {
    if (capacity < 0) throw new IllegalArgumentException();
    IntTreeSet res = new IntTreeSet();
    res.initFromSortedUnique(src, capacity, coloringType);
    return res;
  }

  /**
   * To create {@code IntTreeSet} with the specified capacity use
   * {@link com.almworks.integers.IntTreeSet#createFromSortedUnique(IntIterable, int, com.almworks.integers.IntTreeSet.ColoringType)}
   * @return {@code IntTreeSet} with elements from {@code src}.
   * If {@code src} is a {@code IntSizedIterable}, capacity of new set equals to {@code src.size()}.
   */
  public static IntTreeSet createFromSortedUnique(IntIterable src) {
    return createFromSortedUnique(src,
        IntCollections.sizeOfIterable(src, 0), ColoringType.BALANCED);
  }
  ...
  // todo optimize, if keys small, it's cheaper create new IntArray and use fromSortedList(..)
  // K* log(S + K) -> K*log(K) + (S + K)
  @Override
  public void addAll(IntList keys) {
    addAll((IntSizedIterable)keys);
  }

  public void addAll(IntSizedIterable keys) {
    modified();
    int[] ps = prepareAdd(keys.size());
    for (IntIterator ii : keys) {
   ...
   * */
  protected boolean include0(int key) {
    return include0(key, prepareAdd(1));
  }

  private boolean include0(int key, int[] ps) {
    int x = myRoot;
  ...
  private int createNode(int key) {
    int x;
  ...
  private void maybeGrow(int n) {
    ...
    if (futureSize > myKeys.length) {
      ...
      myKeys = IntCollections.ensureCapacity(myKeys, futureSize);
  ...
  /**
   * This method rebuilds this IntTreeSet, after that it will use just the amount of memory needed to hold size() elements.
  ...
  /**
   * This method is similar to {@link IntTreeSet#compactify()},
   ...
   *       <li>{@link ColoringType#BALANCED} colors every 4th non-last levels red, similar to {@link IntTreeSet#compactify()}.
  ...
  private void initFromSortedUnique(IntIterable src, int capacity, ColoringType coloringType) {
    IntArray buf = new IntArray(capacity + 1);
    buf.add(NIL_DUMMY_KEY);
    buf.addAll(src);
    int size = buf.size() - 1;
    int[] newKeys = buf.extractHostArray();
    assert IntCollections.isSortedUnique(false, newKeys, 1, size) == 0;
   ...
   */
  private void initFromPreparedArray(int[] newKeys, int usedSize, ColoringType coloringType) {
  ...
  public IntIterator iterator() {
    return failFast(new IntIndexedIterator(new IntArray(myKeys), new LURIterator()));
  }

  public IntIterator tailIterator(int fromElement) {
    return failFast(new IntIndexedIterator(new IntArray(myKeys), new LURIterator(fromElement)));
  }
  ...
  protected boolean exclude0(int key) {
   ...
   * */
  public void retain(IntSortedSet set) {
  ...
  private boolean exclude0(int key, int[] parentsStack) {
  ...
  private static int log(int base, int n) {
    int ret = 0;
  ...
  final StringBuilder dumpArrays(int problematicNode) {
    ...
    int intestKey = max(abs(getUpperBound()), abs(getLowerBound()));
    int keyWidth = max(log(10, intestKey), 3);
