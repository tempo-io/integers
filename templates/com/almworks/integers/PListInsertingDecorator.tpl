...
/**
 * Inserting decorator for {@code #E#List} designed
 * to accumulate insertions into a very large list and then use it as a read-only list.
 * Accumulation should be done with {@link #insert(int, #e#)}.
...
public class #E#ListInsertingDecorator extends Abstract#E#ListDecorator {
  private final Int#E#ListMap myInserted;

  public #E#ListInsertingDecorator(#E#List base) {
    this(base, new Int#E#ListMap());
  }
   ...
   * Note that in most cases you'll want to use {@link ##E#ListInsertingDecorator(com.almworks.integers.#E#List)}
   * which constructs a decorator without any insertions and
   * add elements through {@link #insert(int, #e#)}. Use this method if you have previously collected the insertions.
   * <br>Examples:
   * <table>
   ...
   */
  public #E#ListInsertingDecorator(#E#List base, Int#E#ListMap inserted) throws IllegalArgumentException {
    super(base);
    if (inserted.size() != 0) {
  ...
  private class LocalIterator extends Abstract#E#ListIndexIterator {
    ...
    private Int#E#Iterator myInsertedIterator;
