...
public class LongListInsertingDecoratorTests extends LongListChecker<LongListInsertingDecorator> {
  ...
  protected List<LongListInsertingDecorator> createLongListVariants(long... values) {
    ...
    res.add(new LongListInsertingDecorator(array, new IntLongListMap(IntArray.create(array.size()), LongArray.create(lastVal))));

    if (values.length > 1) {
      // ~[...]~
      array = LongArray.copy(array);
      long firstVal = array.removeAt(0);
      res.add(new LongListInsertingDecorator(array, new IntLongListMap(IntArray.create(0, values.length - 1), LongArray.create(firstVal, lastVal))));
    }

    // ~[...]
    array = LongArray.copy(values);
    long firstVal = array.removeAt(0);
    res.add(new LongListInsertingDecorator(array, new IntLongListMap(IntArray.create(0), LongArray.create(firstVal))));
    ...
    if (pos != 0) {
      ...
      res.add(new LongListInsertingDecorator(array, new IntLongListMap(IntArray.create(pos), LongArray.create(posVal))));
    }
  ...
  public void testIndexSimple() {
    ...
    LongListInsertingDecorator tst = new LongListInsertingDecorator(source, new IntLongListMap());
  ...
  public void testMapConstructor() {
    ...
    LongListInsertingDecorator tst = new LongListInsertingDecorator(source,
        new IntLongListMap(IntArray.create(0, 3), LongArray.create(-20, 20)));
    CHECK.order(tst, -20, 2, 4, 20, 6);

    tst = new LongListInsertingDecorator(source, new IntLongListMap());
    CHECK.order(source, tst);

    tst = new LongListInsertingDecorator(source,
        new IntLongListMap(IntArray.create(0, 4), LongArray.create(-20, 20)));
    CHECK.order(tst, -20, 2, 4, 6, 20);

    try {
      new LongListInsertingDecorator(source,
          new IntLongListMap(IntArray.create(0, 5), LongArray.create(-20, 20)));
      fail();
    ...
    try {
      new LongListInsertingDecorator(source,
          new IntLongListMap(IntArray.create(-2, 4), LongArray.create(-20, 20)));
      fail();
    } catch (IllegalArgumentException _) {
      // ok
    }
